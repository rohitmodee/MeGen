...
DART_model = DART_Net(128, 128, 256, 128, 32).to("cuda")
...
class InteractionReward(MolecularReward):
    ...
    def Get_TAD(self, coord):
        ...
        atm_i = torch.tensor([i.reshape(3) for i in desc[0][0]]).to("cuda")
        atm_j = pad_sequence([torch.tensor(i) for i in desc[0][1]], batch_first=True).to("cuda")
        atm_k = pad_sequence([torch.tensor(i) for i in desc[0][2]], batch_first=True).to("cuda")
        atm_l = pad_sequence([torch.tensor(i) for i in desc[0][3]], batch_first=True).to("cuda")
