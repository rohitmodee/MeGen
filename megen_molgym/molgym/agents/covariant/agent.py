...
class CovariantAC(AbstractActorCritic):
    ...
    def step(self, observations: List[ObservationType], actions: Optional[np.ndarray] = None) -> dict:
        ...
        batch_size = len(invariats)
        num_atoms = data["num_atoms"].cpu().numpy()
        # focus: (batches, 1)
        if actions is not None:
            focus = torch.round(actions[:, :1]).long()
        elif self.training:
            # focus = focus_dist.sample().unsqueeze(-1)
            focus = torch.from_numpy(np.random.randint(num_atoms, size=(batch_size, 1))).to(self.device)
            # focus = torch.randint(high=data["num_atoms"].item(), size=(batch_size, 1), device=self.device)
        else:
            focus = torch.from_numpy(np.random.randint(num_atoms, size=(batch_size, 1))).to(self.device)
        ...
        if actions is not None:
            ...
            element = torch.ones(size=(batch_size, 1), device=self.device).long()
        else:
            element = torch.ones(size=(batch_size, 1), device=self.device).long()
